...
// === Tier 7: Antist custom layers ===
import { briefing as japan } from './sources/japan.mjs';
import { briefing as fx } from './sources/fx.mjs';
import { briefing as positions } from './sources/positions.mjs';
import { briefing as aiinfra } from './sources/aiinfra.mjs';
...
const SOURCE_TIMEOUT_MS = parseInt(process.env.SOURCE_TIMEOUT_MS || '45000', 10); // GDELT/OFAC need >30s
...
export async function fullBriefing() {
  ...
  const allPromises = [
    ...
    // Tier 7: Antist custom layers
    runSource('Japan', japan),
    runSource('FX', fx),
    runSource('Positions', positions),
    runSource('AIInfra', aiinfra),
  ];
  ...
  // A source that returned an object is not a source that returned data.
  // Audit the payloads so degradation is stated, never absorbed into the count.
  output.health = auditSources(output);
  output.crucix.sourcesTrulyOk = output.health.trueOkCount;

  console.error(`[Crucix] Sweep complete in ${totalMs}ms — ${output.crucix.sourcesOk}/${sources.length} returned an object`);
  console.error(`[Crucix] ${output.health.report}`);
  return output;
}
