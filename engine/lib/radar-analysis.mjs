...
ACTION FIELD = a short evaluation of THIS item from three fixed lenses, always in this order and with these labels, one short sentence each, specific to the item (never generic):
  (1) humanism (现代世俗人文主义): what it means for human dignity, rights and welfare; free and unfree systems are not equivalent.
  (2) exit (逃离主义): whether/how one can reduce dependence on the system and keep the right to exit — migrate, self-host, decouple, portable skills/assets.
  (3) AI-centrism (AI本主义): what it implies for AI becoming the new structural center (capability integration → structural self-sufficiency → self-legislation; humans lose the center, not necessarily their existence).
Format per language: zh「人文：…；逃离：…；AI本：…」 en「Humanism: …; Exit: …; AI-centrism: …」 ja「人文：…；離脱：…；AI本：…」. No filler such as "keep watching" or generic advice; say something only this item's evidence supports.
