...
// The strip used to show a vendor label only, so gold read "Gold Dec 26" and
// nothing told you which symbol a tile tracked. Show the ticker, then the name
// in the page's own language.
function qName(q){const n=q&&q.names;return (n&&(n[LANG]||n.en))||q.name||q.symbol;}
function qLabel(q){return `<span class="font-semibold tabular-nums text-foreground/80">${esc(q.symbol)}</span><span class="opacity-60"> · </span><span class="truncate">${esc(qName(q))}</span>`;}
function tileView(q,flash=''){const up=q.changePct>=0;return `<a data-symbol="${esc(q.symbol)}" href="${esc(q.source)}" target="_blank" rel="noopener noreferrer" class="market-tile ${flash}"><div class="flex items-center gap-0 text-[11px] font-medium text-muted-foreground truncate">${qLabel(q)}</div><div class="flex items-baseline justify-between gap-2"><span class="text-lg font-semibold tabular-nums">${Number(q.price).toLocaleString()}</span><span class="text-xs font-medium tabular-nums ${up?'text-up':'text-down'}">${up?'+':''}${q.changePct}%</span></div></a>`;}
function tileEdit(q,item,flash=''){const up=q.changePct>=0;return `<div data-symbol="${esc(item.symbol)}" draggable="true" class="market-tile cursor-move ${item.hidden?'opacity-50':''} ${flash}"><div class="flex items-center justify-between gap-1"><span class="flex items-center gap-0 text-[11px] font-medium text-muted-foreground truncate">⠿&nbsp;${qLabel(q)}</span><span class="flex items-center gap-0.5"><button data-hide="${esc(item.symbol)}" title="hide/show" class="btn btn-ghost h-5 w-5 p-0 text-xs leading-none">${item.hidden?'◉':'◌'}</button><button data-del="${esc(item.symbol)}" title="remove" class="btn btn-ghost h-5 w-5 p-0 text-xs leading-none text-destructive">✕</button></span></div><div class="flex items-baseline justify-between gap-2"><span class="text-lg font-semibold tabular-nums">${Number(q.price).toLocaleString()}</span><span class="text-xs font-medium tabular-nums ${up?'text-up':'text-down'}">${up?'+':''}${q.changePct}%</span></div></div>`;}
