...
function filter(){const query=search?.value.toLowerCase()||'';let shown=0;document.querySelectorAll('[data-event]').forEach(e=>{const visible=(category==='all'||e.dataset.category===category)&&e.textContent.toLowerCase().includes(query);e.hidden=!visible;if(visible)shown++;});const no=document.querySelector('[data-no-results]');if(no)no.hidden=shown!==0;}
